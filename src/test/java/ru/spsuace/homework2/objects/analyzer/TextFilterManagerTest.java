...
//
    ...
    public void analyzeCustomFilter() {
        TextFilterManager manager = new TextFilterManager(new TextAnalyzer[]{TextAnalyzer.createCustomTextAnalyzer()});
        assertEquals("CUSTOM", manager.analyze("Переходи по ссылке https://vk.com/al_im.php").toString());
        assertEquals("GOOD", manager.analyze("").toString());
        assertEquals("GOOD", manager.analyze(null).toString());
        assertEquals("GOOD", manager.analyze("Скажите код из смс :-( ").toString());
        assertEquals("CUSTOM", manager.analyze("http://www.google.comhttp://www.google.com").toString());
    }
