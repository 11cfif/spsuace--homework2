...
public class MailServiceTest {
    ...
    public void oneSenderOneRecipientOneMail() {
        ...
        BaseMail mail = new Message(testSender, testRecipient, "text");
    ...
    public void oneSenderOneRecipientManyMails() {
        ...
        for (int i = 0; i < 10; i++) {
            BaseMail mail = new Message(testSender, testRecipient, "text" + i);
            testMailList.add(mail);
            expectedMailList.add(mail);
        }
        for (int i = 0; i < 10; i++) {
            BaseMail mail = new Salary(testSender, testRecipient, (i + 1) * 10000);
            testMailList.add(mail);
            expectedMailList.add(mail);
        }
    ...
    public void ManySenderManyRecipientManyMails() {
        ...
        BaseMail mail1 = new Message(testSender1, testRecipient1, "text1");
        testMailList.add(mail1);
        expectedMailList.add(mail1);

        BaseMail mail2 = new Salary(testSender2, testRecipient2, 10000);
        testMailList.add(mail2);

        BaseMail mail3 = new Salary(testSender2, testRecipient1, 20000);
